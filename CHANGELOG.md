...
- **lanname is now pinned to `>=0.5.0,<0.6`**, up from `>=0.2.0,<0.3`. Three
  ...
  one with no default route, quietly behaved as `--resolve dns` does. A failed
  multicast send raised out of the mDNS lookup and carried past NetBIOS, which
  was never reached. The worker caught it and went on, and reverse DNS and
  `--hosts` named what they always had, so what was lost was both probes
